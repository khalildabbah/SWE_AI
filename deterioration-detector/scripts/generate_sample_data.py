...
=====================
Writes a small, synthetic LABEVENTS-shaped CSV that mirrors the real MIMIC-III
schema, so anyone (e.g. a grader) can run the whole project end-to-end WITHOUT
the 1.8 GB credentialed MIMIC download.

The synthetic cohort is hand-designed so EVERY dashboard panel is populated:
  - High / Medium / Low risk admissions  -> all triage filter tabs have content
  - 8 core labs per admission, 6 timepoints each -> full lab-trend grid
  - worsening / improving / stable trajectories -> shaped risk-trajectory curves
  - critical + abnormal values -> early-warning alerts fire

The numbers are illustrative only and are NOT real patient data.
...
# Each admission: (subject_id, hadm_id, admit_time, {itemid: 6-point series, oldest->newest}).
...
INTERVAL_H = 8  # hours between successive draws


def flag_for(itemid: float, value: float) -> str:
...
def main() -> None:
    ...
    with OUT.open("w", newline="") as f:
        ...
        for subj, hadm, t0, overrides in ADMISSIONS:
            ...
            for i in range(n_points):
                ...
                for itemid, (name, unit) in LABS.items():
                    ...
                    row_id += 1
                    n_rows += 1
                    w.writerow([row_id, subj, hadm, itemid, charttime,
                                value, value, unit, flag_for(itemid, value)])
    print(f"Wrote {n_rows:,} rows for {len(ADMISSIONS)} admissions -> {OUT}")
